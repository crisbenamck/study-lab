...
const AutomaticModelsSection: React.FC = () => {
  ...
  return (
    <div className="bg-info-50 border border-info-200 rounded-lg p-4 mb-6 theme-transition">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full flex items-start justify-between space-x-3 text-left focus:outline-none"
      >
        <div className="flex items-start space-x-3 flex-1">
          <div className="w-5 h-5 text-info-600 flex-shrink-0 mt-0.5">🤖</div>
          <div className="flex-1">
            <h3 className="font-medium text-info-800">Sistema de Modelos con Fallback Automático</h3>
        ...
        <div className="flex-shrink-0 mt-0.5">
          {isExpanded ? (
            <ChevronUp className="w-5 h-5 text-info-600" />
          ) : (
            <ChevronDown className="w-5 h-5 text-info-600" />
          )}
        </div>
      </button>
